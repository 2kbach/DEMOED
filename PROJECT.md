...
## Changelog
- ✅ **2026-04-21 11:00** — v1.4.0. Replaced ReplayKit recording entirely with a CADisplayLink + AVAssetWriter pipeline. iOS caps ReplayKit at 1920px max dimension (enforced by the `replayd` daemon), so on iPhone 15/16 Pro native 2622×1206, ReplayKit was recording at ~1920×884 and upscaling back. New pipeline snapshots the key window via `drawHierarchy(afterScreenUpdates: false)` on each display refresh, draws into a CVPixelBuffer pool at device-native pixel dimensions, and feeds AVAssetWriter at 50 Mbps H.264. Full native resolution output, no cap.
- ✅ **2026-04-20 17:00** — v1.3.1. Fixed two regressions from 1.3.0: (a) Status bar positioning: `.safeAreaInset(edge: .top)` positions below the Dynamic Island, not at y=0. Reverted to ZStack overlay + `.ignoresSafeArea(.all, edges: .top)` on the FakeStatusBar, and set the WKWebView's scrollView.contentInset.top to 62 so page content doesn't hide under it. (b) Capture resolution: for screenshots in Fullscreen mode, now using `WKWebView.takeSnapshot` (native resolution) composed with a SwiftUI `ImageRenderer`-rendered FakeStatusBar image, instead of drawHierarchy which downsamples WKWebView. For videos, the writer is now created lazily using the first sample buffer's actual `CMVideoFormatDescription` dimensions, and bitrate bumped to 50 Mbps — so output matches exactly what ReplayKit delivers with no re-encoding scale.
...
**2026-04-21 (v1.4.0)** — After v1.3.1 the video was still blurry, so we searched online for the root cause. Found that iOS enforces a 1920-pixel max-dimension cap on ReplayKit recordings via the `replayd` daemon (see the iDownloadBlog writeup on ReplayKit Max and the FlashyWrappers SDK blog). iPhone 15/16 Pro native is 2622×1206, so every ReplayKit recording was being downsampled to ~1920×884 and stretched back — no amount of writer-side bitrate could recover that. The only non-jailbreak fix is to bypass ReplayKit entirely. Implemented a CADisplayLink-driven recording pipeline: on each display refresh, snapshot the key window via `drawHierarchy(afterScreenUpdates: false)` directly into a CVPixelBuffer from the adaptor's pool (no intermediate UIImage copy), then `adaptor.append` with a timestamp derived from `link.timestamp - startTime`. Pixel buffer is sized at the device's actual pixel dimensions (width × height × screen.scale). Output is 50 Mbps H.264 High Profile MP4 at true native resolution. Tradeoff accepted: renders every frame on the main thread; for a few-minute demo capture this is fine, but longer recordings might heat up or drop frames under load.
